...
class SuggestionPortal extends React.Component {
  ...
  adjustPosition = () => {
    ...
    // readOnly properties is use to prevent slate onChange update
    // when selection position insert an element
    // need to use setTimeout to manually set it back to false
    // because browser API is async
    if (this.matchTrigger()) {
      this.props.callback.readOnly = true
      const rect = position()
    ...
    setTimeout(() => {
      this.props.callback.readOnly = false
    }, 1)
  }
